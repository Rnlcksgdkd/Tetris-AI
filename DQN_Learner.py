...
class DQN_Learner:
    def __init__(self, version, load_mode=False, filename=[], epsi=1, ):
    ...
    # 각 블록타입마다 하나씩 총 7개의 신경망 생성
    def build_models(self):

        models = []
        for i in range(self.numBlocktype):
            models.append(self.build_model(self.action_space[i]))
    ...
    # 학습 결과를 저장하는 함수
    def save_LearnResult(self, version, TestNum):
        ...
        # 학습 신경망 파라미터 저장
        for i in range(self.numBlocktype):
            self.models[i].save_weights("./save_model/DQN_v%f_%d_blocktype%d.h5" % (version, TestNum, i))

        # 학습 결과를 csv 파일로 저장
        res_df = pd.DataFrame(columns=['Version', 'TestNum', 'ExplorationSteps', 'LearningTime', 'Qmax', 'AliveSteps', 'AvgReward', 'Clear pt'])
        res = [version, TestNum, self.exploration_steps, self.LearningTime, Qmax, AliveSteps, AvgReward, LineClear]
        df = pd.DataFrame([res], columns=['Version', 'TestNum', 'ExplorationSteps', 'LearningTime', 'Qmax', 'AliveSteps', 'AvgReward', 'Clear pt'])
        res_df = pd.concat([res_df, df], ignore_index=True)

        res_df.to_csv('DQN_v%f_Result_.csv' % (version) )
    ...
    def DQN_Learn(self, GLOBAL_STEPS):
        ...
        while True:
            ...
            # 학습 400 K 마다 중간 저장 / 학습 완료시 학습 종료
            ...
            while True:
                ...
                # 2. Action  ->  게임 진행 및  next_state / Reward 받음
                next_state = self.tetris.line_step(action)
                next_history = np.reshape([next_state], (1, 10, 20, 1))
                reward = self.tetris.reward
                step += 1
                global_step += 1

                ## 주의할 점 : Line_step 함수는 지정위치로 갈 때 까지 게임을 진행시키는 함수임. 즉 블록을 이동시키는 과정에서 중간 개입은 불가능함
                ##            - 게임 리셋도 line_Step 함수 안에서 자동으로 일어남 , 리셋신호 확인을 위해서는 Rsingal 체크해야됨
                ##            - AI 행동에 제한을 두는 Buffer 불가 , AI 는 이동시킬 수 있는 위치라면 시간의 제한 없이 블록을 이동시켜서 놓을 수 있음


                # 3. 학습에 필요한 변수를 메모리에 저장
                self.append_sample(history, action, reward, next_history, type)


                # 4. 신경망 학습
                # 일정 Step (2K) 마다 학습을 반복 (2K) (GPU 적용)
                if global_step > 15000 and global_step % 2000 == 0:

                    for __ in range(2000):
                        self.train_model()

                    # 일정 Step (10K) 마다 Target 신경망 업데이트
                ...
                if self.tetris.Rsignal:
                    print( 'episode:{}, score:{}, epsilon:{}, global step:{}, loss : {},  avg_qmax:{}, memory:{}, avg_reward:{}, game_Step:{}'.
                            format(epi, score, self.epsilon, global_step, self.avg_loss / float(self.batch_size),
                                   self.avg_q_max / float(step), self.getTotalMemory(), float(avg_reward / step),
                                   step))
